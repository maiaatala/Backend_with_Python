...
class Book(Base):
    ...
    author = relationship(
        "Author", backref=backref("books", cascade="all, delete, delete-orphan")
    )

    # many books to one publisher
    publisher_id = Column(Integer, ForeignKey("publishers.id"))
    publisher = relationship(
        "Publisher", backref=backref("books", cascade="all, delete, delete-orphan")
    )

    # many books to many categories
    category = relationship(
        "Category", secondary="categories_books", back_populates="book"
    )
